...
export default function ConsultasList({ dataInicio, dataFim }) {
  ...
  useEffect(() => {
    const buscarConsultas = async () => {
      ...
      try {
        ...
        // Se o backend informar total de páginas, setar aqui:
  ...
  const exportarPDF = () => {
    ...
    const linhas = consultas.map((c) => [
      c.paciente_nome || c.paciente,
      c.medico_nome || c.medico,
      c.data,
      c.hora,
      c.status
    ]);
        ...
        <table className="w-full border-collapse border border-gray-300">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="border border-gray-300 px-3 py-1 text-center">Ações</th>
            </tr>
          </thead>
          <tbody>
            {consultas.map(c => (
              <tr key={c.id} className="hover:bg-gray-100">
                ...
                <td className="border border-gray-300 px-3 py-1">{c.paciente_nome || c.paciente}</td>
                <td className="border border-gray-300 px-3 py-1">{c.medico_nome || c.medico}</td>
